class DOMNodeCollection {
  ...
  on(mustBeHandled, callback) {
    this.elements.forEach((el) => {
     el.addEventListener(mustBeHandled, callback);
     const eventKey = `jqliteEvents-${mustBeHandled}`;
  ...
  off(Handled) {
    this.elements.forEach((el) => {
      const eventKey = `jqliteEvents-${Handled}`;
      if (el[eventKey]) {
        el[eventKey].forEach(callback => {
          el.removeEventListener(Handled, callback);
        });
      }
      el[eventKey] = [];
    });
